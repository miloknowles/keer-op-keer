...
export function ChatWindow({ roomId, playerId, players, onClose }: Props) {
  ...
  function resolveName(msg: RoomChatRow) {
    ...
    return players.find((p) => p.id === msg.player_id)?.display_name ?? 'Unknown'
  }

  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-3 pb-2 shrink-0 flex items-center justify-between">
        <div className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Chat</div>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 pb-2 flex flex-col gap-2 min-h-0">
        ...
        {messages.map((msg) => {
          ...
          if (!name) {
            return (
              <p key={msg.id} className="text-xs text-gray-400 italic text-center">{msg.message}</p>
            )
          }
          ...
          return (
            <div key={msg.id} className={`flex flex-col gap-0.5 ${isMe ? 'items-end' : 'items-start'}`}>
              <div className="flex items-baseline gap-1.5">
                {!isMe && <span className="text-[10px] font-semibold text-gray-500">{name}</span>}
                <span className="text-[10px] text-gray-300">{time}</span>
                {isMe && <span className="text-[10px] font-semibold text-gray-500">you</span>}
              </div>
              <div
