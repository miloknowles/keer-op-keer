...
describe('computeScore — row bonus', () => {
  it('awards row bonus only to first completer', () => {
    ...
    const player2 = makePlayer({ id: 'p2', crossed_cells: ['H-Q', ...rowPCells] })
    const allPlayers = [player1, player2]
...
describe('computeScore — color bonus', () => {
  ...
  it('awards subsequent color bonus to second completer', () => {
    ...
    const player2 = makePlayer({ id: 'p2', crossed_cells: ['H-P', ...greenCells] })
    const allPlayers = [player1, player2]
...
describe('computeScore — star penalty', () => {
  ...
  it('zero star penalty when all stars crossed', () => {
    const allStars = ['B-S', 'C-P', 'D-S', 'E-V', 'F-P', 'G-U', 'H-R', 'I-V', 'J-Q', 'L-P', 'M-S', 'N-U']
...
describe('computeScore — total', () => {
  it('total is the sum of all components', () => {
    ...
    const expected =
      ...
      Object.values(result.colors as Record<string, number>).reduce((a, b) => a + b, 0) +
      result.stars
