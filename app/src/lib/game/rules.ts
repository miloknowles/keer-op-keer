...
import { getCell, getAdjacentCells, isValidPlacement, getConnectedRegion } from './sheet'
...
export function validateBombCells(config: BoardConfig, cells: string[]): ValidationResult {
  if (cells.length !== 4) return fail('bomb requires exactly 4 cells')
  for (const key of cells) {
    if (!getCell(config, key)) return fail(`cell ${key} does not exist on board`)
  }
  ...
  if (maxCol - minCol !== 1 || maxRow - minRow !== 1) return fail('bomb cells must form a 2×2 block')
  // Ensure all four corners of that block are present
  const keys = new Set(cells)
  const c1 = config.grid.columns[minCol], c2 = config.grid.columns[maxCol]
  const r1 = config.grid.rows[minRow], r2 = config.grid.rows[maxRow]
  for (const c of [c1, c2]) {
    for (const r of [r1, r2]) {
      if (!keys.has(`${c}-${r}`)) return fail('bomb cells must form a complete 2×2 block')
    }
  }
  return ok()
}
...
export function validateColorNumberPick(
  ...
  const { color_die, number_die, declared_color, declared_number, cells } = pick
  ...
  if (round >= 3 && !isActivePlayer) {
    ...
    if (activePick.type === 'color_number') {
      if (color_die === activePick.color_die) return fail('color die already used by active player')
      if (number_die === activePick.number_die) return fail('number die already used by active player')
    }
  }
  ...
  if (!colorIsWild && colorFace !== declared_color) {
    return fail(`declared_color (${declared_color}) does not match color die face (${colorFace})`)
  }

  // Declared number is valid
  if (declared_number < 1 || declared_number > 5) return fail('declared_number must be 1–5')
  if (!numberIsWild && numberFace !== String(declared_number)) {
    return fail(`declared_number (${declared_number}) does not match number die face (${numberFace})`)
  }

  // Cell count matches declared number
  if (cells.length !== declared_number) {
    return fail(`must cross off exactly ${declared_number} cells, got ${cells.length}`)
  }
  ...
  for (const key of cells) {
    ...
    if (cell.color !== declared_color) return fail(`cell ${key} is not color ${declared_color}`)
  ...
  if (pick.bomb_cells && pick.bomb_cells.length > 0) {
    ...
    for (const bk of pick.bomb_cells) {
      if (allCrossedAfterPick.has(bk)) return fail(`bomb cell ${bk} is already crossed`)
    }
  }
...
export function validateSpecialPick(
  ...
  switch (roll.special) {
    ...
    case 'fill': {
      if (cells.length === 0) return fail('fill must include at least one cell')
      ...
      for (const key of cells) {
        if (getCell(config, key)!.color !== color) return fail('fill cells must all be the same color')
      }
      // Cells must form a connected region
      const connected = getConnectedRegion(config, color, cells[0], player.crossed_cells)
      const connectedSet = new Set(connected)
      for (const key of cells) {
        if (!connectedSet.has(key)) return fail(`fill cells are not a fully connected same-color region`)
      }
      if (connected.length !== cells.length) {
        return fail('fill must cross off the entire connected region, not a subset')
      }
      // At least one cell must be adjacent to existing region (or start condition)
      const anyValid = cells.some(key => isValidPlacement(config, key, player.crossed_cells))
      if (!anyValid) return fail('fill region must be adjacent to existing crossed region')
      break
    }

    case 'three_in_a_row': {
      if (cells.length !== 3) return fail('three_in_a_row requires exactly 3 cells')
      const rows = cells.map(k => k.split('-')[1])
      if (new Set(rows).size !== 1) return fail('three_in_a_row cells must all be in the same row')
      for (const key of cells) {
        if (!getCell(config, key)) return fail(`cell ${key} does not exist on board`)
        if (crossedSet.has(key)) return fail(`cell ${key} is already crossed`)
    ...
    case 'two_stars': {
      ...
      for (const key of cells) {
        const cell = getCell(config, key)
        if (!cell) return fail(`cell ${key} does not exist on board`)
        if (cell.special !== 'star') return fail(`cell ${key} is not a star cell`)
        if (crossedSet.has(key)) return fail(`cell ${key} is already crossed`)
      }
      break
    }
...
export function canPass(
  ...
  for (let ci = 0; ci < 3; ci++) {
    for (let ni = 0; ni < 3; ni++) {
      ...
      // Colors to try
      const colorsToTry: string[] = colorIsWild ? ['p', 'o', 'y', 'g', 'b'] : [colorFace]
      // Numbers to try
      const numbersToTry: number[] = numberIsWild ? [1, 2, 3, 4, 5] : [parseInt(numberFace)]
      ...
      for (const dc of colorsToTry) {
        for (const dn of numbersToTry) {
          ...
          if (potentialCells.length >= dn) {
            ...
            if (round >= 3 && !isActivePlayer && activePick?.type === 'color_number') {
